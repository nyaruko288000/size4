...
proc main() =
  ...
  for filename in args:
    ...
    for line in lines(filename):
      ...
      try:
        ...
        let s4 = obj["s4"].getInt()
        
        if not atlas.hasKey(fp):
          atlas[fp] = MergedEntry(s2: s2, s3: s3, s4: s4, axioms: @[])
  ...
  entries.sort(proc (a, b: (string, MergedEntry)): int =
    result = cmp(a[1].s4, b[1].s4)
    if result == 0: result = cmp(a[1].s3, b[1].s3)
  ...
  f.writeLine("# Groupoid Axiom Equivalence Classes")
  f.writeLine("# Sorted by (S4, S3, S2) ascending")
  f.writeLine("")
  
  for (fp, entry) in entries:
    f.writeLine("--------------------------------------------------")
    f.writeLine("ID: " & fp & " | S2: " & $entry.s2 & " | S3: " & $entry.s3 & " | S4: " & $entry.s4)
